...
public class CameraWrapper {
    ...
    public NativeCamera getmNativeCamera() {
    ...
    public RecordingSize getSupportedRecordingSize(int width, int height) {
        CameraSize recordingSize = previewSize;
        if (recordingSize == null)
            recordingSize = getOptimalSize(getSupportedVideoSizes(VERSION.SDK_INT), width, height);
        if (recordingSize == null) {
        ...
        record_width = recordingSize.getWidth();
        record_height = recordingSize.getHeight();
    ...
    public void configureForPreview(int viewWidth, int viewHeight) {
        ...
        Log.d("test", previewSize.getWidth() + "");
        Log.d("test", previewSize.getHeight() + "");
//        params.setPreviewSize(640, 480);
        params.setPreviewSize(previewSize.getWidth(), previewSize.getHeight());
        params.setPreviewFormat(ImageFormat.NV21);

//        params.setRecordingHint(true);//告知相机一直采用视频采集状态，稳定帧率，
